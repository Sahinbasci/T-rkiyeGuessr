...
export default function HomeClient() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      // After setMoves resets the limit, restore the actual movesUsed from server state.
      // This prevents the player from navigating again after a page refresh.
      const myPlayer = room.players?.[playerId];
      if (myPlayer && myPlayer.movesUsed > 0) {
        syncMovesUsed(myPlayer.movesUsed);
      }
    };
  ...
  useEffect(() => {
    if (room?.status === "playing" && room.currentRound !== prevRoundRef.current) {
      // Track completed round for interstitial frequency cap (skip round 1 init)
